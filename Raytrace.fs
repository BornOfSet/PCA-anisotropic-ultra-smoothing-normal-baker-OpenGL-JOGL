...
/***********************************************************************************************************************************
    //在进入这个函数之前，我希望你记住，开头我们就开始算下一个格子了，因此请保证你在调用此函数前已经算过了old analyse,也就是上一个格子
    //这里只是简单地根据position来判断这个格子在不在整体aabb内
    vec3 NextgridEntry = Orig + Dir*(LastgridExit+0.0001);
    vec3 originaldata = ceil((NextgridEntry-GlobalMin)/unitsize);
    bool X = originaldata.x < 0 || originaldata.x > xcount;
    bool Y = originaldata.y < 0 || originaldata.y > ycount;
    bool Z = originaldata.z < 0 || originaldata.z > zcount;
    if(X||Y||Z){
        ...
        vec3 GVTminraw = (bbmin - Orig)/Dir;
        vec3 GVTmax = max(GVTmaxraw, GVTminraw);
        HitInfo analyse = InsideOneGrid(which);
        if(!analyse.boolean){
            //如果这个格子里面没有碰到任何一个图元
            //一轮调用只可能走一条分支。如果第一次就进入递归，那么...........
            //最终输出的就是最终的递归的结果
            return SearchForNext(Dir, min( GVTmax.x, min(GVTmax.y,GVTmax.z)) , GlobalMin, OldAnalyse);
        }else{
            //碰到图元了，它一定是最近的，那么返回
            return analyse;
        }
...
void main(){
    ...
    float Lambert = dot(LIGHTDIRECTION, interpolation.normal);
    scene = vec4(Lambert,Lambert,Lambert, 1);
}