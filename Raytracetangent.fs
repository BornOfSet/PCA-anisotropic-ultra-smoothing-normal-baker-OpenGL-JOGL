...
HitInfo Hit(vec3 Dir, TriangleStruct prim){
    ...
    vec3 reached = OrigPS + float(c)*Dir;
...
HitInfo SearchForNext(float Tdir, vec3 Dir, float Control, vec3 GlobalMin, HitInfo OldAnalyse){
    ...
    while(safety < 100){
        ...
        //请注意， OrigPS + Dir*(LastgridExit+0.0001);这段代码是控制退出的根本保障
        vec3 NextgridEntry = OrigPS + Dir*(LastgridExit+Tdir*0.0001);
        ...
        if(X||Y||Z){
            ...
            vec3 GVTmaxraw = (bbmax - OrigPS)/Dir;
            vec3 GVTminraw = (bbmin - OrigPS)/Dir;
...
void main(){
    ...
    vec3 Dir = -normalize(DirecPS);
    vec3 VTmin = (globalmin - OrigPS)/Dir;
    vec3 VTmax = (globalmax - OrigPS)/Dir;
    VTmin = min(VTmax, VTmin);
    float Tmin = max(  VTmin.x ,max( VTmin.y,VTmin.z) );
    vec3 where = OrigPS + Dir * Tmin;//这个where考虑到了低模发射器的坐标在高模grid里面的情况，这个情况下它会向后找到延长线最处进入的位置，也就是将光线从射线扩展为直线
	...
	vec3 where_id = max(vec3(0,0,0),ceil((where-globalmin)/unitsize)-1.0);
    ...
    float where_id_1D = where_id.x + where_id.y*xcount + where_id.z*xcount*ycount;

    GridHelper which = grids[int(where_id_1D)];
    vec3 bbmax = vec3(which.bbmax[0], which.bbmax[1], which.bbmax[2]);
    vec3 bbmin = vec3(which.bbmin[0], which.bbmin[1], which.bbmin[2]);
    vec3 GVTmaxraw = (bbmax - OrigPS)/Dir;//V是vector的意思。G指的是grid
    vec3 GVTminraw = (bbmin - OrigPS)/Dir;
    //经过测试，which.start正确了
    ...
    vec3 GWhereLeave = OrigPS + Dir*GTmax;
    vec3 GWhereEnter = OrigPS + Dir*GTmin;
    ...
    vec3 B = cross(flatNormal,Tangent);
    //把B取负数等同于y=1-y
    mat3 TBN = mat3(normalize(Tangent), normalize(B), normalize(flatNormal));//插值后不是归一的

    vec3 tspaceN = transpose(TBN) * normalize(interpolation.normal);
