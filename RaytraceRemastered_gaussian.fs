        ...
        for(int b = int(y_begin); b <= int(y_end); b++){
            for(int c = int(z_begin); c <= int(z_end); c++){
                ...
                for(int i= int(which.start);i< int(which.end);i++){
                    ...
                    sum += GaussianKernel(X - where) * angle;
                    if(length(X-where)<=sphereradius){
                        norm += angle;
                    }
                }
            }
...
HitInfo Hit(vec3 Dirf, TriangleStruct prim, int id){
    ...
    return HitInfo(prim, c, reached, a>=double(0.) && a<=double(1.) && b>=double(0.) && b<=double(1.) && a+b<=double(1.), primrefs[id], ivec3(0));
