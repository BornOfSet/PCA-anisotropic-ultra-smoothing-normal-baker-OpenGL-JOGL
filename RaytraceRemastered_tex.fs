...
HitInfo RemasteredSearchForNext(ivec3 where_id, vec3 Dir){
...
void main(){
    ...
    vec3 worldspace;
    vec4 tex = texture(text0, uv);
    vec3 tangentspace = tex.xyz * 2 - 1;
    vec3 Ta = normalize(T);
    vec3 No = normalize(N);
	...
	where_id = anyOfVec(Orig,globalmin)&&anyOfVec(globalmax,Orig)?shootAbroad:shootAbroad;


    HitInfo analyse = RemasteredSearchForNext(where_id, Dir);
    Vertex interpolation = Barycentric(analyse);

//卡渲可用的版本把-改成+
    vec3 nNo = normalize(mat3(-normalize(newT), -normalize(cross(newT,  No)), No) * tangentspace);
    float nLengthTN = dot(Ta, nNo);
    vec3 nalignedN = nLengthTN * nNo;
    vec3 nnewT = Ta - nalignedN;
    mat3 noTBN = mat3(normalize(nnewT), normalize(cross(nnewT, nNo)), nNo);
    vec3 tspaceN = transpose(noTBN) * normalize(interpolation.normal);
