...
HitInfo Hit(vec3 Dirf, TriangleStruct prim){
    ...
    if(a>=double(0.) && a<=double(1.) && b>=double(0.) && b<=double(1.) && a+b<=double(1.)){
        inside = true;
    }
...
HitInfo SearchForNext(float Tdir, vec3 Dir, float Control, vec3 GlobalMin, HitInfo OldAnalyse){
    ...
    while(safety < 100){
        ...
        //当解决了Hit函数的精度问题，这里就是限制条件，导致少部分区域无法检测到
        //Lastgridexit+0：原地踏步，全部无法检测
        //Lastgridexit+Tdir*unitsize，跳步严重，跳过了真正需要检测的区域
        //可能需要改成传统的画线方法
        ...
        if(X||Y||Z){
            ...
            bool contain = false;
            if(anyOfVec(Orig - bbmin)&&anyOfVec(bbmax - Orig)){ contain = true;}
...
void main(){
    ...
    //Where -> Orig
	vec3 where_id = min(ivec3(xcount-1,ycount-1,zcount-1),max(ivec3(0,0,0),ivec3(ceil((where-globalmin)/unitsize))-int(1)));
    ...
    //平滑TBN
    vec3 sTa = normalize(T);//这个normalize(T)有问题，会导致极化（菊花）
    //因为对三个中心对称可以相互抵消的向量算中心值，得到的是零向量
    //奇怪，那那些个拉力是哪来的？不应该解码编码抵消吗
    //还要注意，未normalize的矩阵不正交，算法是有问题的
    vec3 sNo = normalize(N);
   // vec3 sBi = normalize(cross(sTa, sNo));
   // mat3 sTBN = mat3(sTa,sBi,sNo);
