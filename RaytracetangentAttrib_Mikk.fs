...
HitInfo SearchForNext(float Tdir, vec3 Dir, float Control, vec3 GlobalMin, HitInfo OldAnalyse, int L){
...
void main(){
    ...
    //修正的
	ivec3 where_id = ivec3(min(floor((where-globalmin)/unitsize),vec3(xcount-1,ycount-1,zcount-1)));
    ...
    vec3 len = min(floor(abs(GWhereLeave - GWhereEnter)/unitsize), vec3(xcount,ycount,zcount));
    int lenI = int(length(len));
    ...
    //这里加上判断
    if(!analyse.boolean){
        analyse = SearchForNext(Tdir, Dir, GTmax , globalmin, analyse, lenI);
    }
    ...
    //scene.xyz = sNo;
    scene.xyz = normalize(interpolation.overlay);
